...
for cc in enumerate(clay_cont): # Loop over sc
    ...
    cpal = sns.color_palette("mycolormap", n_colors=len(sat)+1)
...
norm = mpl.colors.Normalize(vmin=min(sat_filt),vmax=max(sat_filt))
...
# plt.colorbar()
plt.savefig('Spectrum_Clay' + str(clay_cont[0]) + '_raw' + str(raw), dpi=300)
